...
def generate_sequential_graph_dataset(feature_graph_list, indices, ocel_object, k):
    ...
    for gid in tqdm(idx):
        ...
        edge_pairs = [(index_series[i],index_series[i+1])for i in range(0,len(index_series)-1)]
        cart_set = [graph.nodes,graph.nodes]
        new_edges = [(n,m) for n,m in itertools.product(*cart_set) if (n.event_id,m.event_id) in edge_pairs]
